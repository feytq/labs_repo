...
Оказывается, что начальный выбор города тоже существенно влияет на работу алгоритма. Если для полного перебора стартовая точка не играет роли (контур все равно замкнут), то для жадного метода смена старта полностью меняет траекторию обхода. Из-за своей близорукости алгоритм на каждом шаге выбирает локально дешевые ребра: при удачном выборе вершины цепочка решений дает близкий к оптимальному результат (качество 90–100%), а при неудачном старте он быстро исчерпывает дешевые пути в начале маршрута и в конце вынужден возвращаться на базу по самым дорогим ребрам матрицы, из-за чего на одном и том же графе качество может резко проседать до 10% и ниже.
